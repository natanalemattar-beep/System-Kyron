...
- Custom form components for registration (prefix selectors, RIF/Cédula lookup), reusable document upload, and Venezuela geography data for cascading dropdowns (Estado → Municipio → Parroquia/Ciudad).
- All auth pages (login, register, recuperar-cuenta) use a centralized forced light theme via `src/app/[locale]/(auth)/layout.tsx` — a client component that removes the `dark` class on mount and restores it on unmount. This ensures consistent light appearance for all auth flows regardless of the user's theme preference.
- Asesoría Contable registration page uses brand colors: dark/light blues and dark/light/fluorescent greens. No violet or amber. Plan cards use lightblue, green, darkblue, and neon (lime) color schemes.
- Dark/light theme system with smooth Apple-style transitions (700ms cubic-bezier). Default theme is dark. Theme toggle in header animates between sun/moon icons with spring easing. CSS variables adapt via `.dark` class (next-themes). Theme preference stored in localStorage as `kyron-theme`. Dark mode uses deep navy palette (hsl 224° base), light mode uses clean blue-tinted white (hsl 210° base). Liquid glass Apple-style effects throughout: `liquid-glass-apple`, `glass-card`, `kyron-surface`, `liquid-glass`, `liquid-glass-subtle` — all use CSS custom properties (`--glass-bg`, `--glass-border`, `--glass-shadow`, `--glass-highlight`) that auto-adapt per theme. All landing sections have `dark:` gradient variants for consistent dark backgrounds.
- Asesoría Contable verification step (step 3) offers three verification channels: Email, SMS, and WhatsApp. The user selects a method and the code is sent via that channel.
...
- **API Routes:** Over 60 API routes for authentication, KPIs, and CRUD operations. Includes `/api/solicitudes` for generic service requests and `/api/arqueo-caja` for cash register closings. All 40+ action buttons across every module now wired to real API persistence (zero toast-only fake saves remain). All fake `setTimeout` simulations eliminated — 14 pages converted to real `POST /api/solicitudes` calls.
- **Demo Banner:** Global dismissable amber banner (`DemoBannerProvider` + `DemoBannerSpacer`) in root locale layout. Shows "Fase de Demostración — Los datos mostrados son ilustrativos." with dynamic padding that disappears when banner is dismissed.
