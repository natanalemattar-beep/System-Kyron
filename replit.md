...
- **Reserva de Datos Panel:** Reusable `ReservaDatosPanel` component (`src/components/telecom/reserva-datos-panel.tsx`) shows per-line free data reserves for app-only access, with a "Modo Forzoso" toggle that converts the reserve to paid general-use data at $0.02/MB. Includes animated progress bars, cost tracking, and confirmation dialogs with explicit charge warnings. Used in both `/mi-linea` (personal, after usage gauges) and `/flota-empresarial` (enterprise, before CONATEL compliance panel). Props: `tipo: 'personal' | 'empresa'`, optional `lineas` array.
