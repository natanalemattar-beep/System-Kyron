...
- **Performance Optimizations:** Lazy loading, dynamic imports, loading skeletons, Next.js image optimization, CSS-based animations, and WebP images. Background animations are optimized using pure CSS. All `backdrop-blur` and `backdrop-filter` effects removed globally from glass/surface classes for GPU performance. Continuous CSS pseudo-element animations (liquid-shimmer, liquid-refract, prismatic-sweep) disabled. Background orbs hidden on mobile via media query. Landing page below-fold sections lazy-loaded with IntersectionObserver.
...
- **Sistema de Planes con Límites (AI + Alertas de Cortesía):** 4 planes (Starter $0, Profesional $29, Empresarial $79, Kyron MAX $199/mes). Cada plan incluye AI y alertas de cortesía con límites progresivos. 17 recursos controlados: consultas AI, chat AI, alertas fiscales, alertas regulatorias, facturas, empleados nómina, clientes CRM, documentos legales, líneas telecom, reportes, exportaciones Excel, consultas RIF, declaraciones asistidas, blockchain proofs, simulador de multas, usuarios, almacenamiento. Solo Kyron MAX tiene todo ilimitado. DB table: `uso_plan` (tracking mensual por usuario). API: `/api/planes` (GET vista=planes|mi-plan|uso|verificar|comparar, POST accion=usar-recurso|cambiar-plan). Responde 429 con mensaje de upgrade cuando se alcanza el límite.

- **Facturación Homologada SENIAT (Providencia 0071):** Sistema de facturación 100% conforme a la Providencia Administrativa SNAT/2011/00071. Tabla `facturas` incluye: numero_control (auto-generado consecutivo), serie, tipo_documento (FACTURA/NOTA_DEBITO/NOTA_CREDITO/ORDEN_ENTREGA/GUIA_DESPACHO), condicion_pago (contado/credito), base_imponible/exenta/no_sujeta discriminadas, alícuotas IVA (general 16%, reducida 8%, adicional 31%, exento), IGTF 3%, retenciones IVA/ISLR, datos emisor (RIF, razón social, domicilio fiscal), referencia a factura original para NC/ND (Art. 18), leyenda "sin derecho a crédito fiscal", soporte dual moneda con tasa BCV. Tabla `factura_items` incluye: código, unidad, tipo_gravamen (gravado/exento/no_sujeto). Páginas: Centro de Facturación (`/facturacion`), Modelo Factura SENIAT (`/modelo-factura`), Nota de Débito (`/nota-debito`), Nota de Crédito (`/nota-credito`). API: `/api/facturas` (GET con filtros tipo/estado/tipo_documento, POST con auto-generación numero_control + validación RIF). Fecha emisión en formato 8 dígitos (DD/MM/AAAA).
- **Inmutabilidad Fiscal de Documentos (SENIAT):** Los documentos fiscales emitidos (facturas, NC, ND) son completamente inmutables conforme a la Providencia SNAT/2011/00071. Implementación multi-capa: (1) Hash fiscal SHA-256 generado al emitir, (2) Columna `inmutable=true` + `emitida_at` timestamp, (3) Trigger PostgreSQL `trg_proteger_factura` que bloquea UPDATE en campos fiscales (numero_factura, numero_control, total, subtotal, monto_iva, base_imponible, rif_emisor, fecha_emision, etc.), (4) Trigger `trg_bloquear_delete_factura` que impide DELETE de facturas inmutables, (5) Trigger `trg_bloquear_delete_items` que impide eliminar items de facturas emitidas. Transiciones permitidas: emitida→cobrada/pagada (registro de cobro), emitida→anulada (solo si se referencia una NC válida). Borradores (`estado='borrador'`) son editables hasta ser emitidos. UI muestra candado verde + tooltip SENIAT en documentos inmutables, y aviso rojo pre-emisión. La copia del cliente y la de la empresa son idénticas e inalterables.
