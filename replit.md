...
**AI & External Services (Replit AI Integrations + direct key fallback):**
AI providers use Replit's native AI integrations (`AI_INTEGRATIONS_*` env vars) as primary source, with fallback to direct API keys (`ANTHROPIC_API_KEY`, etc.) if integration vars are not set. DeepSeek uses direct key only (no Replit integration available). Gmail uses Nodemailer SMTP.
- **AI Clients:** Anthropic (`src/ai/anthropic.ts`), OpenAI (`src/ai/openai.ts`), Gemini (`src/ai/gemini.ts`), DeepSeek (`src/ai/deepseek.ts`). Each client checks `AI_INTEGRATIONS_*_API_KEY` + `AI_INTEGRATIONS_*_BASE_URL` first, then falls back to direct key. Gemini integration requires `httpOptions: { apiVersion: '', baseUrl }`.
...
- **Health check:** `/api/integrations-health` runs live tests against all 4 AI providers + Gmail SMTP. Reports key source (replit_integration vs direct_env_var).
- **Replit integration env vars:** `AI_INTEGRATIONS_ANTHROPIC_API_KEY`, `AI_INTEGRATIONS_ANTHROPIC_BASE_URL`, `AI_INTEGRATIONS_OPENAI_API_KEY`, `AI_INTEGRATIONS_OPENAI_BASE_URL`, `AI_INTEGRATIONS_GEMINI_API_KEY`, `AI_INTEGRATIONS_GEMINI_BASE_URL`
- **Direct env vars (fallback):** `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `DEEPSEEK_API_KEY`, `GMAIL_USER`, `GMAIL_APP_PASSWORD`, `DATABASE_URL`
...
- **Enterprise API Architecture (v3.5.0):** Standardized enterprise-grade API infrastructure:
  - **Typed Error Hierarchy** (`src/lib/api-errors.ts`): `AppError` → `ValidationError`, `AuthenticationError`, `AuthorizationError`, `NotFoundError`, `ConflictError`, `RateLimitError`, `BadRequestError`, `ServiceUnavailableError`. Each with HTTP status codes, error codes, and optional field-level details.
  - **Standardized Response Envelope** (`src/lib/api-response.ts`): All APIs return `{ success: true, data, meta? }` or `{ success: false, error: { code, message, details? } }`. Paginated responses include `meta.pagination` with `page`, `limit`, `total`, `totalPages`, `hasNext`, `hasPrev`.
  - **Zod Request Validation** (`src/lib/api-validation.ts`): `validateBody()`, `validateQuery()`, `validateParams()` with Zod schemas. Returns typed data or throws `ValidationError` with field-level error details.
  - **Composable Route Handler** (`src/lib/api-handler.ts`): `apiHandler(fn, opts)` wraps route handlers with automatic: error catching → typed responses, session auth (configurable), rate limiting, request ID generation, performance timing, structured logging.
  - **Pagination** (`src/lib/pagination.ts`): `parsePagination()` (offset-based, max 100), `parseSort()` (whitelist-based), `buildPaginatedQuery()`.
  - **Structured Logger** (`src/lib/logger.ts`): JSON structured logging in production, readable format in dev. Supports levels (debug/info/warn/error), child loggers, request context, timed operations with threshold warnings.
  - **Frontend API Client** (`src/lib/api-client.ts`): Type-safe `api.get/post/put/patch/delete` with automatic response envelope parsing, `ApiError` class with status/code/details.
  - **Middleware Enhancements**: `x-request-id` header on all API responses for tracing, `x-response-time` for performance monitoring. Standardized 401 responses use new error envelope format.
  - **Migration pattern**: Routes using new `apiHandler` (clientes, campanas, stats). Frontend consumers use `json.data ?? json.oldKey` for backward compatibility during gradual migration.
...
- **Session Timeout:** Auto-logout after configurable inactivity (default 6 min). Warning dialog appears 60 seconds before expiry with countdown, "Seguir activo" / "Cerrar sesión" buttons. Configurable in Settings (mode: auto/manual, duration: 3/5/6/10/15/30 min). Config stored in `kyron-session-timeout` localStorage key, live-synced via `kyron-session-timeout-changed` custom event. Files: `src/hooks/use-session-timeout.ts`, `src/components/session-timeout-dialog.tsx`. Mounted globally in `src/components/providers.tsx`.
- **Settings Page:** Allows user configuration of animation, navigation, notification, session timeout, fiscal, and company data.
