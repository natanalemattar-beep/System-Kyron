...
- Dark mode has been permanently removed. The app forces light theme via `forcedTheme="light"` in ThemeProvider. All `dark:` Tailwind classes have been stripped from the entire codebase. All pages (including login portal, landing, admin) use light backgrounds. The light theme uses a soft gray-blue palette (not pure white) for comfortable viewing. ThemeToggle renders null. Only exception: carnet card designs and OG image keep dark gradients intentionally.
