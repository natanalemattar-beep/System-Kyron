...
- Features premium landing page (16 sections total): hero with rotating text + floating metric cards + animated gradient orbs, trust numbers banner with animated counters, "¿Por qué System Kyron?" differentiator section with comparison table vs generic ERPs, interactive modules grid (24 modules with category filter tabs), "Cómo Funciona" 4-step timeline, features timeline, showcase gallery with hover glow effects, services showcase, compliance section, integrations strip, about us, comments, CTA with guarantee badges, FAQ, and refined footer with tech badges. Intuitive mobile sidebar, redesigned Login/Dashboard, and dynamic pricing display.
...
- **Performance Optimizations:** Utilizes lazy loading, dynamic imports (framer-motion `dynamic()` on 4 heavy dashboard pages), loading skeletons, Next.js image optimization, CSS animations (no GPU-heavy filter blur transitions in framer-motion — removed for performance), WebP images, adaptive performance (3-tier: high/medium/low — medium tier now disables particles and blur backgrounds), smart loading screens, IntersectionObserver, in-memory TTL cache with LRU eviction (middleware session cache: 500 max entries, 20s TTL, smart eviction at 80% capacity), reduced background blur element sizes, optimized database queries with 11+ custom indexes (factura_items, libro_diario_lineas, plan_cuentas, clientes, etc.), `React.memo` on heavy chart components (OverviewChart, DailyTasksChart), batch INSERT for factura_items (eliminates N+1), `Promise.any` for parallel BCV rate fetching (3 sources race), `optimizePackageImports` for 32+ packages, and extended API cache TTLs (stats 60s, BCV rate 30min, tasa 10min).
