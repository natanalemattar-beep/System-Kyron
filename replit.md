...
- **Dashboard Redesign (v2.8.5+):** All three dashboards (Dashboard Empresa, Dashboard RRHH, Dashboard Personal) were completely redesigned with a fresh, compact UI. Dashboard Empresa uses ALL real API data from `/api/dashboard` including variaciones (month-over-month), tasaBCV, cuentasCobrar/cuentasPagar as objects, facturasEsteMes, clientesActivos, inventarioBajoStock, notificacionesNoLeidas, and a real 12-month `chartMensual` from the database. Includes: time-of-day greeting (Buenos días/tardes/noches + first name), live health score indicator (0-100% based on utilidad, facturas vencidas, stock, etc.), mini-sparkline charts inside KPI cards showing 12-month trend, clickable stat cards linking to their respective modules, live clock that updates every minute, hover-lift animations on all cards. Dashboard Personal features: greeting with first name, verification level system (Básico→Plata→Oro→Platino based on document count) with animated progress bar, clickable KPI cards, notification count badges, and ChevronRight indicators on interactive elements. Dashboard RRHH includes searchable employee directory, colored bar chart, average salary KPI.
