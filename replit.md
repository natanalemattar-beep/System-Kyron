...
- **Theme-Aware Screenshots:** Landing page screenshots (showcase and services sections) use the `ThemeImage` component (`src/components/ui/theme-image.tsx`) which renders both dark and light variants stacked with CSS `dark:opacity-0`/`dark:opacity-100` classes, matching the same pattern used for the hero background images. Screenshot assets stored as `*-dark.jpg` and `*-light.jpg` pairs in `public/images/landing/`.
