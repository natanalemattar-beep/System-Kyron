...
- Dark mode has been permanently removed. The app forces light theme via `forcedTheme="light"` in ThemeProvider. All `dark:` Tailwind classes have been stripped from the entire codebase. All pages (including login portal, landing, admin) use light backgrounds. The light theme uses a soft green-blue palette (hsl 180/175/185° hues) with gradient backgrounds across all layouts (`bg-gradient-to-br from-[hsl(170,...)] via-background to-[hsl(210,...)]`). Each layout has floating emerald/teal/cyan/sky gradient orbs for visual depth. ThemeToggle renders null. Only exception: carnet card designs and OG image keep dark gradients intentionally.
