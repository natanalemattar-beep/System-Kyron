...
- **Real RIF/Cédula Validation (`src/lib/validacion-venezuela.ts`):** Replaced AI-generated fake data with real mathematical validation. RIF uses the Venezuelan modulo-11 check digit algorithm (weights `[4,3,2,7,6,5,4,3,2]`). Cédula uses strict format validation. Both `/api/rif/consulta` and `/api/cedula/consulta` endpoints now perform DB-only lookups and return honest "not found" responses instead of fabricating data with AI. Registration API (`/api/auth/register`) enforces `validarRIF()` and `validarFormatoCedula()` server-side. Legacy AI-sourced SAIME records are filtered out (`source != 'ia'`).
