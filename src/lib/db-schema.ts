...
async function createProyectosTables() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS permisos_legales (
      ...
      user_id           INT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      tipo              TEXT NOT NULL
                        CHECK (tipo IN ('licencia','permiso','registro','certificacion','habilitacion','solvencia','autorizacion','municipal','ambiental','sanitario','bomberos','trabajo','intt','seniat','sunagro','sasa','minpet','conatel','sapi','propiedad_intelectual','otro')),
...
async function seedAutomationRules(): Promise<void> {
  const rules = [
    ...
    {
      name: 'Alertas de Contratos Laborales',
      description: 'Detecta contratos laborales próximos a vencer (30, 15, 7 días) y genera notificaciones',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 24, label: 'Diario' },
    ...
    {
      name: 'Alertas de Cuentas por Pagar',
      description: 'Detecta pagos a proveedores próximos a vencer (7, 3, 1 día) y pagos vencidos',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 12, label: 'Cada 12 horas' },
    ...
    {
      name: 'Alertas de Cumpleaños de Empleados',
      description: 'Notifica el día anterior al cumpleaños de cada empleado activo',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 24, label: 'Diario' },
      action_type: 'hr_birthday_alerts',
      module: 'rrhh',
    },
    {
      name: 'Alertas de Evaluación / Período de Prueba',
      description: 'Detecta empleados en período de prueba próximo a finalizar (90 días) y solicita evaluación',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 24, label: 'Diario' },
      action_type: 'hr_evaluation_alerts',
      module: 'rrhh',
    },
    {
      name: 'Alertas de Saldo Elevado de Clientes',
      description: 'Detecta clientes con saldos pendientes que superan el umbral configurado',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 12, label: 'Cada 12 horas' },
      action_type: 'credit_limit_alerts',
      module: 'finanzas',
    },
    {
      name: 'Alertas de Stock Bajo',
      description: 'Detecta productos con stock actual por debajo del mínimo configurado',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 6, label: 'Cada 6 horas' },
      action_type: 'low_stock_alerts',
      module: 'inventario',
    },
    {
      name: 'Alertas de Progreso KPI',
      description: 'Detecta metas KPI con progreso inferior al 50% cuando ha transcurrido más del 75% del período',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 6, label: 'Cada 6 horas' },
      action_type: 'kpi_progress_alerts',
      module: 'inventario',
    },
    {
      name: 'Alertas de Telecomunicaciones',
      description: 'Detecta facturas telecom por vencer, líneas con contrato próximo a vencer y consumo excesivo de datos',
      trigger_type: 'schedule',
      trigger_config: { interval_hours: 24, label: 'Diario' },
      action_type: 'telecom_alerts',
      module: 'telecom',
    },
    {
      name: 'Alertas de Seguridad',
      description: 'Detecta intentos de login fallidos, cambios de permisos/roles y actividad inusual fuera de horario laboral',
      trigger_type: 'schedule',
