        ...
        <div className="space-y-12 pb-20 px-6 md:px-10">
            ...
            <header className="flex flex-col md:flex-row justify-between items-end gap-10 border-l-4 border-secondary pl-8 py-2 mt-10">
                <div className="space-y-2">
                    ...
                    <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white uppercase leading-none italic-shadow">Portal de <span className="text-secondary italic">Sostenibilidad</span></h1>
                ...
                <div className="flex gap-2">
                    <Button variant="outline" className="h-12 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest border-white/10 bg-white/5 text-white" onClick={() => handleSimulation("Función disponible en la próxima actualización.")}>
                    ...
                    <motion.div key={kpi.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                        <Card className="glass-card border-none bg-white/[0.02] p-2 rounded-[2rem]">
                            <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0 p-6">
                                ...
                                <div className={cn("p-2 rounded-lg border border-white/5 shadow-inner", kpi.bg)}>
                                    <kpi.icon className={cn("h-4 w-4", kpi.color)} />
                                </div>
                            </CardHeader>
                            <CardContent className="p-6 pt-0">
                                <div className="text-3xl font-black italic text-white tracking-tighter leading-none mb-2">{kpi.value}</div>
                                <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest">{kpi.desc}</p>
                            </CardContent>
                        </Card>
                    </motion.div>
                ))}
            ...
            <div className="grid gap-10 lg:grid-cols-12">
                ...
                <Card className="lg:col-span-4 glass-card border-none rounded-[3rem] bg-white/[0.01] flex flex-col items-center justify-center p-10 text-center shadow-2xl">
                    <CardHeader className="p-0 mb-8">
                        <CardTitle className="text-sm font-black uppercase tracking-[0.4em] text-white/30 italic">Distribución de Residuos</CardTitle>
                    </CardHeader>
                    ...
                    <CardFooter className="p-0 grid grid-cols-2 gap-4 w-full">
                        {distributionData.map(item => (
                            <div key={item.name} className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-black uppercase text-white/40 tracking-widest">{item.name} {item.value}%</span>
                            </div>
                        ))}
                    </CardFooter>
                </Card>
            ...
            {/* --- History & Map --- */}
            <div className="grid gap-10 lg:grid-cols-12">
                <Card className="lg:col-span-7 glass-card border-none rounded-[3rem] bg-white/[0.01] overflow-hidden shadow-2xl">
                    <CardHeader className="p-10 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                        <div className="space-y-1">
                            <CardTitle className="text-xl font-black uppercase italic tracking-tighter text-white">Historial de Reciclaje</CardTitle>
                            <p className="text-[9px] font-bold uppercase opacity-30 tracking-widest">Validación de Pesaje por IA</p>
                        </div>
                        <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-white/5" onClick={() => handleSimulation("Función de exportación próximamente.")}><Filter className="h-4 w-4" /></Button>
                    </CardHeader>
                    <CardContent className="p-0">
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-white/[0.02] border-none">
                                    <TableHead className="pl-10 py-5 text-[9px] font-black uppercase tracking-widest text-white/30">Fecha</TableHead>
                                    <TableHead className="py-5 text-[9px] font-black uppercase tracking-widest text-white/30">Tipo</TableHead>
                                    <TableHead className="py-5 text-[9px] font-black uppercase tracking-widest text-white/30">Peso</TableHead>
                                    <TableHead className="text-right pr-10 py-5 text-[9px] font-black uppercase tracking-widest text-white/30">Eco-Créditos</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {recyclingHistory.map((row) => (
                                    <TableRow key={row.id} className="border-white/5 hover:bg-white/[0.02] transition-colors group">
                                        <TableCell className="pl-10 py-6 text-[10px] font-bold text-white/30 uppercase">{row.fecha}</TableCell>
                                        <TableCell className="py-6 font-black uppercase text-xs text-white/80 group-hover:text-white">{row.tipo}</TableCell>
                                        <TableCell className="py-6 font-mono text-xs text-white/40">{row.peso}</TableCell>
                ...
                <Card className="lg:col-span-5 glass-card border-none rounded-[3rem] bg-white/[0.01] p-1 shadow-2xl relative overflow-hidden">
                    <div className="p-10 space-y-6">
                        <h3 className="text-xl font-black uppercase italic tracking-tighter text-white">Smart Bins Cercanos</h3>
                        <p className="text-[10px] font-bold uppercase opacity-30 tracking-widest">Estado de los Puntos de Recolección</p>
                        
                        <div className="relative aspect-square bg-black/40 rounded-[2.5rem] border border-white/5 overflow-hidden group">
                            {/* SVG Map Mockup */}
                            <svg viewBox="0 0 400 400" className="w-full h-full opacity-20">
                                <path d="M50 100 Q 150 50 350 150 T 350 350" stroke="#475569" strokeWidth="2" fill="none" />
                                <path d="M100 350 Q 200 300 300 350" stroke="#475569" strokeWidth="2" fill="none" />
                            </svg>
                            ...
                            <div className="absolute bottom-6 left-6 right-6">
                                <AnimatePresence mode="wait">
                                    {selectedBin ? (
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="p-4 bg-black/80 backdrop-blur-md rounded-2xl border border-white/10 flex justify-between items-center">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-black uppercase text-secondary italic">Smart Bin {selectedBin}</p>
                                                <p className="text-[8px] font-bold text-white/40 uppercase">🟢 Activo • 60% Disponible</p>
                                            </div>
                                        ...
                                        <p className="text-center text-[8px] font-bold text-white/20 uppercase tracking-widest italic">Seleccione un punto en el mapa</p>
                                    )}
                                </AnimatePresence>
                            </div>
                        </div>
            ...
            <div className="grid gap-10 lg:grid-cols-12">
                <Card className="lg:col-span-8 glass-card border-none rounded-[3rem] bg-white/[0.01] overflow-hidden shadow-2xl">
                    <CardHeader className="p-10 border-b border-white/5 flex flex-row justify-between items-center bg-white/[0.01]">
                        <div className="space-y-1">
                            <CardTitle className="text-xl font-black uppercase italic tracking-tighter text-white">Mercado de Eco-Créditos</CardTitle>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {marketOrders.map((order, i) => (
                                    <TableRow key={i} className="border-white/5 hover:bg-white/[0.02] transition-all">
                                        ...
                                        <TableCell className="py-6 font-black italic text-white/80">{order.qty}</TableCell>
                                        <TableCell className="py-6 font-mono font-bold text-secondary">{order.price}</TableCell>
                                        <TableCell className="py-6 text-[10px] font-black uppercase text-white/40">{order.user}</TableCell>
                ...
                <Card className="lg:col-span-4 glass-card border-none rounded-[3rem] bg-white/[0.01] p-10 shadow-2xl flex flex-col justify-between">
                    <div className="space-y-6">
                        <h3 className="text-sm font-black uppercase tracking-[0.4em] text-white/30 italic">Valor ECR (USD)</h3>
                        <div className="h-40 w-full bg-black/40 rounded-2xl p-4 border border-white/5 relative overflow-hidden group">
                            <ResponsiveContainer width="100%" height="100%">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between text-[10px] font-black uppercase text-white/20"><span>Capitalización:</span> <span className="text-white/60">$ 1.5M</span></div>
                            <div className="flex justify-between text-[10px] font-black uppercase text-white/20"><span>Volumen 24h:</span> <span className="text-white/60">45k ECR</span></div>
                        </div>
                    </div>
                    <Button variant="outline" className="w-full h-12 rounded-xl mt-8 border-white/10 bg-white/5 font-black text-[9px] uppercase tracking-widest text-white/40 group hover:text-white" onClick={() => handleSimulation("Abriendo estadísticas completas...")}>ANÁLISIS TÉCNICO <ArrowUpRight className="ml-2 h-3 w-3 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all"/></Button>
                </Card>
            </div>

            {/* --- Achievements & Certificates --- */}
            <div className="grid gap-10 lg:grid-cols-12">
                <Card className="lg:col-span-6 glass-card border-none rounded-[3rem] bg-white/[0.01] p-10 shadow-2xl">
                    <h3 className="text-xl font-black uppercase italic tracking-tighter text-white mb-8 flex items-center gap-4">
                            ...
                            <div key={ach.id} className="space-y-3">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="space-y-0.5">
                                            <p className={cn("text-xs font-black uppercase tracking-tight", ach.unlocked ? "text-white" : "text-white/20")}>{ach.title}</p>
                                            <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest">{ach.unlocked ? "Desbloqueado" : `Meta: ${ach.target}`}</p>
                                        </div>
                                    </div>
                                ...
                                {!ach.unlocked && (
                                    <div className="space-y-1.5">
                                        ...
                                        <Progress value={(ach.current / parseInt(ach.target)) * 100} className="h-1.5 bg-white/5 border border-white/5" />
                                    </div>
                                )}
                            </div>
                        ))}
                        ...
                        {[
                            ...
                            <div key={i} className="p-6 rounded-2xl bg-white/5 border border-white/5 flex justify-between items-center group hover:border-primary/20 transition-all">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        <p className="text-xs font-black uppercase italic text-white/90">{cert.name}</p>
                                        <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest">{cert.date} • REF: {cert.ref}</p>
                                    </div>
                                </div>
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-white/5" onClick={() => handleSimulation("Generando PDF de certificado...")}><Download className="h-4 w-4" /></Button>
                                    <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-white/5" onClick={() => handleSimulation("Compartiendo certificado en red...")}><Share2 className="h-4 w-4" /></Button>
                                </div>
                            </div>
                        ))}
                    </div>
                    <CardFooter className="p-0 pt-10 flex justify-center border-t border-white/5 mt-10">
            ...
            <Dialog open={isMarketModalOpen} onOpenChange={setIsMarketModalOpen}>
                <DialogContent className="rounded-[3rem] bg-black/95 backdrop-blur-3xl border-white/10 p-10">
                    <DialogHeader className="mb-8">
                        ...
                        <DialogTitle className="text-3xl font-black italic uppercase tracking-tighter text-white leading-none">Orden de <br/> Eco-Exchange</DialogTitle>
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-1">Cantidad de Eco-Créditos (ECR)</Label>
                            <div className="relative">
                                <Input type="number" placeholder="0" className="h-14 bg-white/5 border-white/10 rounded-2xl pl-12 text-lg font-black italic text-white" />
                                <Coins className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-secondary" />
                            </div>
                        </div>
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-1">Precio Unitario (USD)</Label>
                            <div className="relative">
                                <Input type="number" placeholder="1.25" className="h-14 bg-white/5 border-white/10 rounded-2xl pl-12 text-lg font-black italic text-white" />
                        ...
                        <div className="p-6 rounded-2xl bg-white/5 border border-white/5 space-y-2">
                            <div className="flex justify-between text-[10px] font-black uppercase text-white/20"><span>Total Proyectado:</span> <span className="text-white/60">$ 0.00</span></div>
                            <div className="flex justify-between text-[10px] font-black uppercase text-white/20"><span>Comisión Ecosistema (1%):</span> <span className="text-white/60">$ 0.00</span></div>
                        </div>
                    </div>
                    ...
                    <DialogFooter className="mt-10 flex flex-col gap-4">
                        ...
                        <Button variant="ghost" className="w-full h-12 text-[10px] font-black uppercase text-white/20 hover:text-white" onClick={() => setIsMarketModalOpen(false)}>Cancelar Operación</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            {/* --- Footer Branding --- */}
            <footer className="mt-20 pt-10 border-t border-white/5 flex flex-col items-center gap-6 text-center opacity-40">
