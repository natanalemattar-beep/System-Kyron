    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({ title: "Procesamiento Completado", description: "Solicitud de plano registrada exitosamente." });
            } else {
                ...
                <Card className="lg:col-span-5 glass-card border-none rounded-2xl bg-white/[0.02] p-10 flex flex-col justify-between">
                    <div className="space-y-8">
                        <div className="space-y-4">
                            ...
                            <p className="text-xs font-medium text-white/40 uppercase leading-relaxed">Suba una imagen del local comercial para que el sistema genere el plano arquitectónico y calcule el presupuesto de acabados.</p>
                        </div>
                    ...
                    {planoGenerado ? (
                        <Card className="glass-card border-none rounded-2xl bg-black/40 overflow-hidden h-full flex flex-col">
                            <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                                <span className="text-[11px] font-semibold uppercase tracking-wider text-primary">Previsualización Técnica</span>
                            ...
                            <div className="flex-grow relative bg-white/5">
                                <Image src={planoGenerado} alt="Plano Técnico" fill className="object-contain p-10 opacity-80" />
                            </div>
                            <CardFooter className="p-8 border-t border-white/5 flex justify-between items-center bg-white/[0.01]">
                                ...
                                <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px] font-bold px-4 h-6 rounded-lg uppercase">Cálculo Preciso</Badge>
                            </CardFooter>
                        </Card>
                    ) : (
