...
export default function AcademiaKyronPage() {
    return (
        <div className="space-y-12 w-full px-6 md:px-16 pb-20">
                    ...
                    <Card key={i} className="glass-card border-none overflow-hidden rounded-[2.5rem] bg-white/[0.02] group flex flex-col">
                        <div className="aspect-video relative overflow-hidden">
                            ...
                            <div className="absolute top-4 left-4">
                                <Badge className="bg-black/60 backdrop-blur-md text-[10px] font-semibold uppercase tracking-widest border-white/10">{course.level}</Badge>
                            </div>
                        </div>
                        <CardHeader className="p-8 pb-4">
                            <CardTitle className="text-xl font-semibold uppercase italic tracking-tight leading-tight text-white/90">{course.title}</CardTitle>
                            <CardDescription className="text-xs font-bold text-white/30 uppercase mt-2">{course.desc}</CardDescription>
                        </CardHeader>
                        <CardContent className="p-8 pt-0 flex-grow">
                            <div className="flex items-center gap-6 mb-6">
                                <span className="flex items-center gap-2 text-[11px] font-bold text-white/40 uppercase tracking-widest"><Clock className="h-3.5 w-3.5" /> {course.duration}</span>
                                <span className="flex items-center gap-2 text-[11px] font-bold text-white/40 uppercase tracking-widest"><BookOpen className="h-3.5 w-3.5" /> 12 Lecciones</span>
                            </div>
                            {course.progress > 0 && (
                                <div className="space-y-2">
                                    <div className="flex justify-between text-[10px] font-semibold uppercase tracking-widest text-primary">
                                        <span>Progreso</span>
                        ...
                        <CardFooter className="p-8 pt-0 mt-auto">
                            <Button className={cn(
                                "w-full h-12 rounded-xl font-semibold text-[10px] uppercase tracking-widest",
            ...
            <Card className="bg-gradient-to-br from-yellow-500 to-amber-600 border-none rounded-2xl p-12 relative overflow-hidden">
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center relative z-10">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        {[
                            ...
                            <div key={i} className="p-6 bg-white/10 rounded-xl border border-white/20 flex flex-col items-center text-center gap-4">
                                <feat.icon className="h-8 w-8 text-white" />
                                <span className="text-[11px] font-semibold uppercase tracking-widest text-white">{feat.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
