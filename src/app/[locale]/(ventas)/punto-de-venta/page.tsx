        ...
        <div className="flex flex-col gap-4 animate-in fade-in duration-500">
            <header className="flex items-center justify-between bg-card/80 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-lg">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-semibold uppercase tracking-tight italic text-primary">System Kyron</h1>
                        <p className="text-[10px] font-bold uppercase tracking-widest opacity-40">Punto de Venta Profesional</p>
                    </div>
                </div>
                 {!activeCashier ? (
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Button onClick={() => setActiveCashier(null)} variant="ghost" size="sm" className="text-[10px] font-bold text-rose-500 uppercase tracking-widest hover:bg-rose-500/10 rounded-lg">Finalizar Turno</Button>
                    </div>
                )}
            </header>
            ...
            <div className="grid lg:grid-cols-3 gap-4 h-full min-h-[calc(100vh-14rem)]">
                         ...
                         {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(product => (
                            <Card key={product.id} onClick={() => addToCart(product)} className="group cursor-pointer hover:border-primary transition-all rounded-[1.5rem] overflow-hidden bg-background/50 border-white/5 relative flex flex-col h-full">
                                ...
                                <div className="p-4 flex-grow flex flex-col justify-between">
                                    ...
                                    <p className="text-sm font-bold text-primary italic tracking-tight">{formatCurrency(product.price, 'USD')}</p>
                                </div>
                            </Card>
                        ))}
                    </div>
                ...
                <Card className="flex flex-col bg-card/60 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 shadow-lg overflow-hidden">
                    <CardHeader className="p-8 pb-4">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-lg font-semibold uppercase italic tracking-tight text-white">Orden Actual</CardTitle>
                            <Badge variant="secondary" className="bg-primary/10 text-primary border-none text-[10px] font-bold">{cart.length} ITEMS</Badge>
                        </div>
                    </CardHeader>
                    <CardContent className="flex-grow overflow-y-auto px-8 space-y-4 custom-scrollbar">
                        {cart.length > 0 ? (
                                ...
                                {cart.map(item => (
                                    <div key={item.id} className="flex items-center justify-between p-4 bg-white/[0.03] border border-white/5 rounded-2xl group hover:bg-white/[0.05] transition-all">
                                        <div className="flex-1 min-w-0 pr-2">
                                            ...
                                            <p className="text-xs font-bold text-primary italic">{formatCurrency(item.price, 'USD')}</p>
                                        </div>
                            ...
                            <div className="flex flex-col items-center justify-center h-full py-20 text-center opacity-20">
                                ...
                                <p className="text-xs font-semibold uppercase tracking-wider mt-4">Cesta Vacía</p>
                            </div>
                        )}
                    </CardContent>
                    <CardFooter className="flex-col !p-8 border-t border-white/10 gap-6 bg-black/40">
                        <div className="w-full space-y-3">
                            <div className="flex justify-between text-[10px] font-semibold uppercase text-white/30"><span>Sub-Total Bruto:</span><span className="text-white/60">{formatCurrency(totalInCurrency, currency)}</span></div>
                            <div className="flex justify-between text-[10px] font-semibold uppercase text-white/30"><span>IVA Aplicado (16%):</span><span className="text-white/60">{formatCurrency(iva, currency)}</span></div>
            ...
            <Dialog open={isReceiptOpen} onOpenChange={setIsReceiptOpen}>
                <DialogContent className="sm:max-w-md rounded-2xl bg-black/95 backdrop-blur-3xl border-white/10 p-10">
                    <DialogHeader className="text-center mb-8">
                        <DialogTitle className="text-3xl font-bold tracking-tight uppercase italic text-white">Transacción Exitosa</DialogTitle>
                        <DialogDescription className="text-[11px] font-bold text-emerald-400 uppercase tracking-widest bg-emerald-500/10 px-4 py-1.5 rounded-full border border-emerald-500/20 w-fit mx-auto mt-4">
                            <Lock className="h-3 w-3 mr-2 inline" /> Sellado en Ledger Digital v2.8.5
                        </DialogDescription>
                    </DialogHeader>
                    <div className="text-center space-y-8">
                        <div className="p-6 bg-white rounded-[2.5rem] shadow-lg relative group">
                            {qrCodeData && (
                            ...
                            <p className="mt-4 text-[10px] font-semibold uppercase text-slate-400 tracking-widest">Código Fiscal Homologado</p>
                        </div>
                        <Button onClick={() => { setIsReceiptOpen(false); setCart([]); }} className="w-full h-14 rounded-2xl font-semibold uppercase text-xs tracking-widest btn-3d-primary shadow-xl italic">NUEVA OPERACIÓN</Button>
                    </div>
                </DialogContent>
            </Dialog>
        </div>
