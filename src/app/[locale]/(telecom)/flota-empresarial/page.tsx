        ...
        <div className="space-y-10 pb-20 px-4 md:px-10 bg-background min-h-screen font-sans text-foreground">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-10 border-l-8 border-primary pl-6 md:pl-10">
                ...
                <div className="flex gap-3 no-print">
                    ...
                    <Button className="bg-primary text-primary-foreground hover:bg-primary/90 h-12 px-10 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl" onClick={() => toast({ title: "Exportando...", description: "Preparando reporte Excel de la flota." })}>
            ...
            <Card className="border-none shadow-sm rounded-[3rem] bg-card overflow-hidden">
                <CardHeader className="p-10 border-b border-border/30 flex flex-col md:flex-row justify-between items-center bg-muted/10">
                    ...
                    <div className="flex items-center gap-3 mt-6 md:mt-0">
                        ...
                        <Button asChild className="h-12 px-6 rounded-xl btn-3d-primary font-black text-[10px] uppercase tracking-widest whitespace-nowrap">
                ...
                <CardContent className="p-0">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-4">
                            ...
                            <p className="text-sm font-black uppercase tracking-widest">Sin líneas registradas</p>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-muted/20 border-none">
                                    <TableHead className="pl-10 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Titular / Operadora</TableHead>
                                    ...
                                    <TableHead className="py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right">Uso Datos</TableHead>
                                    ...
                                    <TableHead className="text-right pr-10 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Monto</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {filtered.map((l) => {
                                    ...
                                    <TableRow key={l.id} className="border-border/30 hover:bg-muted/10 transition-all group">
                                        <TableCell className="pl-10 py-6">
                                            <p className="font-black text-xs text-foreground uppercase italic group-hover:text-primary transition-colors">{l.titular ?? 'Sin asignar'}</p>
                                            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-1">{l.operadora} · {l.tipo_linea}</p>
                                        </TableCell>
                                        <TableCell className="text-center font-mono text-xs font-bold text-primary">{l.numero}</TableCell>
                                        <TableCell className="text-[10px] font-black uppercase text-muted-foreground">{l.plan_contratado ?? '—'}</TableCell>
                                        <TableCell className="text-right">
                                            {limite > 0 ? (
                                                <div className="space-y-1">
                                                    <p className={cn("font-black text-xs italic", isExceeded ? "text-rose-500" : "text-secondary")}>{uso.toFixed(1)} / {limite} GB</p>
                                        ...
                                        <TableCell className="text-center">
                                            {l.activa ? (
                                                <Badge variant="outline" className="text-[8px] font-black uppercase text-secondary border-secondary/20 px-3">ACTIVA</Badge>
                                            ) : (
                                                <Badge variant="destructive" className="text-[8px] font-black uppercase px-3">INACTIVA</Badge>
                                            )}
                                        </TableCell>
                ...
                <CardFooter className="p-10 border-t border-border/30 bg-muted/10 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                        <Button variant="outline" className="rounded-xl h-10 px-6 text-[9px] font-black uppercase tracking-widest border-border" onClick={() => toast({ title: "Redirigiendo...", description: "Abriendo gestor de límites corporativos." })}>Ajustar Límites</Button>
                    </div>
                </CardFooter>
            </Card>
                ...
                <CardHeader className="p-10 border-b border-border/30 bg-gradient-to-r from-primary/5 to-transparent">
                    <div className="flex flex-col md:flex-row md:items-center gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <CardTitle className="text-2xl font-black uppercase italic tracking-tight text-primary">Gestión de Homologación por IMEI</CardTitle>
