    ...
    return (
        <div className="space-y-12 pb-20">
            <header className="border-l-4 border-primary pl-8 py-2 mt-10">
                ...
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight uppercase italic text-white flex items-center gap-6">
                    <ClipboardCheck className="h-10 w-10 text-primary" />
                    Cierre de Caja
                </h1>
                <p className="text-muted-foreground mt-2 font-bold text-[10px] uppercase tracking-wider opacity-40">Registro de Efectivo y Cuadre de Ventas</p>
            </header>

            <div className="grid gap-10 lg:grid-cols-12">
                <div className="lg:col-span-8 space-y-10">
                    <Card className="glass-card border-none rounded-2xl bg-white/[0.01] overflow-hidden shadow-lg">
                        <CardHeader className="p-10 border-b border-white/5 bg-white/[0.01]">
                            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-primary">Estado de Ventas en Sistema</CardTitle>
                        </CardHeader>
                        <CardContent className="p-10 space-y-8">
                            <div className="flex justify-between items-center p-8 bg-white/[0.03] border border-white/5 rounded-xl shadow-inner">
                                <span className="text-[10px] font-semibold uppercase tracking-widest text-white/40">Ventas Registradas:</span>
                                <span className="text-3xl font-bold text-primary">{formatCurrency(resumenSistema.ventasTotales, 'Bs.')}</span>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {[
                                    ...
                                    <div key={p.label} className="p-4 bg-white/[0.01] border border-white/5 rounded-2xl">
                                        <p className="text-[10px] font-semibold uppercase tracking-widest text-white/20 mb-1">{p.label}</p>
                                        <p className="text-xs font-bold text-white/70">{formatCurrency(p.val, 'Bs.')}</p>
                                    </div>
                                ))}
                            </div>
                            <div className="p-8 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex justify-between items-center group hover:bg-emerald-500/20 transition-all">
                                <span className="text-[10px] font-semibold uppercase tracking-wider text-emerald-400">Total Esperado en Caja:</span>
                                <span className="text-3xl font-bold text-emerald-400">{formatCurrency(totalEsperadoEnCaja, 'Bs.')}</span>
                            </div>
                        </CardContent>
                    </Card>

                    <Card className="glass-card border-none rounded-2xl bg-white/[0.01] p-10">
                        <CardHeader className="p-0 mb-10">
                            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-white/40 italic">Registro de Conteo Físico</CardTitle>
                        </CardHeader>
                        <CardContent className="p-0 grid grid-cols-2 md:grid-cols-3 gap-8">
                            {denominacionesBs.map(({ valor, tipo }) => (
                                <div key={valor} className="space-y-3">
                                    <Label className="text-[11px] font-semibold uppercase tracking-widest text-white/20 ml-1">{tipo} de {valor} Bs.</Label>
                                    <Input 
                                        type="number" 
                                        placeholder="0" 
                                        className="h-12 bg-white/5 border-white/10 rounded-xl text-center font-mono font-bold text-primary"
                ...
                <div className="lg:col-span-4 space-y-10">
                    <Card className="glass-card border-none bg-white/[0.02] rounded-2xl p-10 sticky top-24 shadow-lg">
                        <div className="space-y-8">
                            <div className="text-center space-y-2">
                                <p className="text-[10px] font-semibold uppercase tracking-wider text-white/30 italic">Balance Final</p>
                                <div className={cn(
                                    ...
                                    <p className="text-[10px] font-semibold uppercase tracking-widest mb-4 opacity-60">Diferencia</p>
                                    <p className={cn(
                            ...
                            <div className="space-y-3">
                                <Label className="text-[11px] font-semibold uppercase tracking-widest text-white/30 ml-1">Observaciones</Label>
                                <Textarea 
                            ...
                            <Button className="w-full h-16 rounded-2xl font-semibold uppercase text-xs tracking-widest shadow-lg" onClick={handleCerrarCaja}>
                                REGISTRAR CIERRE
                            </Button>
                            
                            <Button variant="outline" className="w-full h-12 rounded-xl border-white/10 bg-white/5 font-bold text-[11px] uppercase tracking-widest text-white/40">
