...
export default function ArqueoCajaPage() {
    ...
    const handleCerrarCaja = () => {
        toast({
            title: "Caja Cerrada",
            description: `Diferencia: ${formatCurrency(diferenciaBs, 'Bs.')}. Registro sellado.`,
    ...
    return (
        <div className="space-y-12 pb-20">
            <header className="border-l-4 border-primary pl-8 py-2">
                ...
                <p className="text-muted-foreground mt-2 font-bold text-[10px] uppercase tracking-[0.6em] opacity-40">Protocolo de Cierre • Auditoría de Punto</p>
            </header>
                ...
                <div className="lg:col-span-8 space-y-10">
                    <Card className="glass-card border-none rounded-[3rem] bg-white/[0.01] overflow-hidden shadow-2xl">
                        <CardHeader className="p-10 border-b border-white/5 bg-white/[0.01]">
                            <CardTitle className="text-sm font-black uppercase tracking-[0.4em] text-primary">Resumen del Turno</CardTitle>
                        </CardHeader>
                        <CardContent className="p-10 space-y-8">
                            <div className="flex justify-between items-center p-8 bg-white/[0.03] border border-white/5 rounded-[2rem] shadow-inner">
                                <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Ventas Totales:</span>
                                <span className="text-3xl font-black italic text-primary">{formatCurrency(resumenSistema.ventasTotales, 'Bs.')}</span>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {[
                                    { label: "Débito", val: resumenSistema.pagosTarjetaDebito },
                                    { label: "Crédito", val: resumenSistema.pagosTarjetaCredito },
                                    { label: "Transf.", val: resumenSistema.pagosPorTransferencia },
                                    { label: "P. Móvil", val: resumenSistema.pagosPagoMovil }
                                ].map(p => (
                            ...
                            <div className="p-8 bg-emerald-500/10 border border-emerald-500/30 rounded-[2rem] flex justify-between items-center group hover:bg-emerald-500/20 transition-all">
                                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-400">Total Esperado (Efectivo + Fondo):</span>
                    ...
                    <Card className="glass-card border-none rounded-[3rem] bg-white/[0.01] p-10">
                        <CardHeader className="p-0 mb-10">
                            <CardTitle className="text-sm font-black uppercase tracking-[0.4em] text-white/40 italic">Conteo de Efectivo</CardTitle>
                        </CardHeader>
                    ...
                    <Card className="glass-card border-none bg-white/[0.02] rounded-[3rem] p-10 sticky top-24 shadow-2xl">
                        <div className="space-y-8">
                            <div className="text-center space-y-2">
                                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/30 italic">Balance de Cierre</p>
                                <div className={cn(
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-widest mb-4 opacity-60">Diferencia (Bs.)</p>
                                    <p className={cn(
                            ...
                            <div className="space-y-3">
                                <Label className="text-[9px] font-black uppercase tracking-widest text-white/30 ml-1">Observaciones de Auditoría</Label>
                                <Textarea 
                                    className="bg-white/5 border-white/10 rounded-2xl text-xs font-bold p-6 min-h-[120px] uppercase placeholder:opacity-30" 
                                    placeholder="Justifique discrepancias..."
                                    value={observaciones}
