        ...
        <div className="space-y-12 pb-20">
            <header className="border-l-4 border-primary pl-8 py-2 mt-10">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-primary/10 border border-primary/20 text-[9px] font-black uppercase tracking-[0.4em] text-primary shadow-glow mb-4">
                    <Wallet className="h-3 w-3" /> NODO DE DIVISAS
                </div>
                <h1 className="text-3xl md:text-5xl font-black tracking-tight text-foreground uppercase leading-none italic-shadow">Exchange <span className="text-primary italic">Corporativo</span></h1>
                <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-[0.6em] opacity-40 mt-2 italic">Billetera Multimoneda • Gestión de Liquidez 2026</p>
            </header>
            ...
            <Card className="glass-card border-none rounded-[3rem] bg-card/40 overflow-hidden shadow-2xl">
                <CardHeader className="p-10 border-b border-border/50 bg-muted/10">
                    <CardTitle className="text-sm font-black uppercase tracking-[0.4em] text-primary italic">Orden de Conversión</CardTitle>
                </CardHeader>
                <CardContent className="p-10 grid md:grid-cols-12 gap-10 items-end">
                    <div className="md:col-span-4 space-y-3">
                        <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 ml-1">Monto a Cambiar</Label>
                        <Input type="number" placeholder="0.00" value={amount} onChange={e => setAmount(Number(e.target.value))} className="h-14 rounded-2xl bg-white/5 border-border font-black italic text-lg" />
                    </div>
                    <div className="md:col-span-3 space-y-3">
                        <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 ml-1">De</Label>
                    ...
                    <div className="md:col-span-3 space-y-3">
                        <Label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 ml-1">A</Label>
                ...
                <CardFooter className="p-10 bg-primary/5 border-t border-border flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-6">
                        <div className="text-center md:text-left">
                            <p className="text-[8px] font-black uppercase text-muted-foreground/40 mb-1">Tasa Aplicada (BCV)</p>
                            <p className="text-xs font-bold text-foreground/80">1 {fromCurrency} = {rate.toFixed(4)} {toCurrency}</p>
                        </div>
                        <div className="h-8 w-px bg-border" />
                        <div className="text-center md:text-left">
                            <p className="text-[8px] font-black uppercase text-primary mb-1">Recibirás (Aprox.)</p>
                            <p className="text-2xl font-black italic text-primary">{formatCurrency(result, toCurrency as any)}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.3em]">
