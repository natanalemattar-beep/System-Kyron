        ...
        <div className="space-y-12 pb-20 px-4 md:px-10">
            <header className="border-l-4 border-primary pl-6 md:pl-8 py-2 mt-10">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-primary/10 border border-primary/20 text-[11px] font-semibold uppercase tracking-wider text-primary mb-4">
                    <Activity className="h-3 w-3" /> CENTRO TRIBUTARIO
                </div>
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground uppercase leading-none">Liquidación <span className="text-primary italic">de IVA</span></h1>
                <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-wider opacity-40 mt-2 italic">Gestión de Impuesto al Valor Agregado • 2026</p>
            </header>

            <div className="flex justify-end">
                <Dialog open={openDialog} onOpenChange={setOpenDialog}>
                    <DialogTrigger asChild>
                        <Button className="h-12 px-8 rounded-xl font-semibold uppercase text-[10px] tracking-widest">
                            <Plus className="mr-2 h-4 w-4" /> NUEVA DECLARACIÓN
                        </Button>
                    </DialogTrigger>
                    <DialogContent className="max-w-lg">
                        <DialogHeader>
                            <DialogTitle className="font-semibold uppercase tracking-tight">Registrar Declaración IVA</DialogTitle>
                        </DialogHeader>
                        <div className="grid grid-cols-2 gap-4 py-4">
                            {[
                                ...
                                <div key={f.key} className={f.key === 'periodo' ? 'col-span-2' : ''}>
                                    <Label className="text-[10px] font-semibold uppercase tracking-widest">{f.label}</Label>
                                    <Input
                                        type={f.type ?? 'text'}
                        ...
                        <Button onClick={handleNuevaDeclaracion} disabled={submitting} className="w-full h-12 font-semibold uppercase text-[10px]">
            ...
            {loading ? (
                ...
                <div className="text-center py-20 text-muted-foreground">
                    ...
                    <p className="font-semibold uppercase tracking-widest text-sm">No hay declaraciones de IVA registradas.</p>
                    <p className="text-xs mt-2 opacity-60">Registra tu primera declaración con el botón de arriba.</p>
                </div>
            ) : (
                <div className="grid lg:grid-cols-3 gap-10">
                    <Card className="lg:col-span-2 glass-card border-none rounded-2xl p-1 shadow-lg bg-card/40">
                        <CardHeader className="p-10 border-b border-border/50">
                            <CardTitle className="text-xl font-semibold uppercase italic tracking-tight text-foreground">Resumen del Periodo: {latest.periodo}</CardTitle>
                        ...
                        <CardContent className="p-10 space-y-10">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div className="p-8 bg-white/[0.03] border border-border rounded-xl shadow-inner space-y-2">
                                    <p className="text-[11px] font-semibold uppercase text-muted-foreground/40 tracking-widest">Débito Fiscal (Ventas)</p>
                                    <p className="text-3xl font-bold text-foreground tracking-tight">{formatCurrency(ivaDebito, 'Bs.')}</p>
                                </div>
                                <div className="p-8 bg-white/[0.03] border border-border rounded-xl shadow-inner space-y-2">
                                    <p className="text-[11px] font-semibold uppercase text-muted-foreground/40 tracking-widest">Crédito Fiscal (Compras)</p>
                                    <p className="text-3xl font-bold text-foreground tracking-tight">{formatCurrency(ivaCredito, 'Bs.')}</p>
                                </div>
                            </div>
                            <div className="p-10 bg-primary/10 border-2 border-primary/20 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-10 group hover:border-primary/40 transition-all">
                                <div className="space-y-2 text-center md:text-left">
                                    <p className="text-[10px] font-bold text-primary uppercase tracking-wider">Monto Neto a Liquidar</p>
                                    <p className="text-5xl font-bold text-primary tracking-tight">{formatCurrency(totalAPagar, 'Bs.')}</p>
                                </div>
                                <Button size="lg" onClick={handleDeclarar} disabled={submitting || latest.estado === 'presentada'} className="w-full md:w-auto h-16 px-12 rounded-2xl font-semibold uppercase text-xs tracking-widest">
                            ...
                            {declaraciones.length > 1 && (
                                <div className="space-y-3">
                                    <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/50">Historial de Declaraciones</p>
                                    {declaraciones.slice(1).map(d => (
                                        <div key={d.id} className="flex justify-between items-center p-4 bg-white/[0.02] border border-border rounded-2xl">
                                            <div>
                                                <p className="text-xs font-semibold uppercase">{d.periodo}</p>
                                                <p className={`text-[10px] font-bold ${d.estado === 'presentada' ? 'text-emerald-500' : 'text-amber-500'}`}>{d.estado.toUpperCase()}</p>
                                            </div>
                                            <p className="text-sm font-bold font-mono">{formatCurrency(parseFloat(d.iva_neto), 'Bs.')}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="space-y-8">
                        <Card className="bg-amber-500/10 border-2 border-amber-500/20 rounded-[2.5rem] p-10 flex flex-col justify-between relative overflow-hidden group shadow-lg">
                            ...
                            <Button variant="outline" className="w-full h-12 rounded-xl border-amber-500/20 bg-amber-500/5 text-amber-600 font-semibold uppercase text-[10px] tracking-widest shadow-inner">
                                <Download className="mr-2 h-4 w-4" /> BAJAR ARCHIVO .TXT
                            </Button>
                        </Card>

                        <Card className="glass-card border-none p-8 rounded-[2.5rem] bg-card/40">
                            <h4 className="text-[11px] font-semibold uppercase tracking-wider text-primary mb-6 flex items-center gap-3 italic">
