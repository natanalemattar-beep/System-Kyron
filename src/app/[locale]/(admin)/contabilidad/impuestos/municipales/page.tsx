...
import { Landmark, Loader2, Inbox, Download } from "lucide-react";
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm font-medium">
            Cálculo de patente de industria y comercio sobre ingresos brutos.
          </p>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-3">
              ...
              <p className="text-xs text-muted-foreground/70">Registre facturas de venta para calcular el impuesto municipal.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {rows.map((f) => (
                  <TableRow key={f.id} className="hover:bg-muted/30 transition-colors">
                    <TableCell className="pl-8 font-mono text-xs font-bold">{f.factura}</TableCell>
