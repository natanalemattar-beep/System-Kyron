...
import { Loader2, CircleCheck as CheckCircle, ArrowRight, ArrowLeft, Eye, EyeOff, Recycle, ShieldCheck, RefreshCw, Leaf } from 'lucide-react';
...
const TIPOS_RESIDUOS = ['Sólidos Urbanos','Peligrosos','Industriales','Electrónicos (RAEE)','Orgánicos','Reciclables (PET/Vidrio/Cartón)','Médicos/Hospitalarios','Mixtos','Otro'];
const CERTIFICACIONES = ['ISO 14001','OHSAS 18001','ISO 50001','Carbono Neutro','Sin certificación','En proceso de certificación'];
const AREAS_IMPACTO = ['Local (Municipio)','Regional (Estado)','Nacional','Internacional'];
const TECNOLOGIAS_CLASIFICACION = ['Manual','Automatizada (Cintas)','IA/Sensores Magnéticos (Ameru)','Robots Clasificadores','Híbrida'];
...
const schema = z.object({
    ...
    certificacion_ambiental: z.string().min(1, 'Seleccione la certificación'),
    ...
    password: z.string().min(8,'Mínimo 8 caracteres').regex(/[A-Z]/,'Una mayúscula requerida').regex(/[0-9]/,'Un número requerido'),
...
export default function RegisterSostenibilidadPage() {
    ...
    const stepFields: Record<number, (keyof FormData)[]> = {
        1: ['razonSocial','rif','telefono','estado_empresa','municipio_empresa','direccion'],
        2: ['tipo_residuos','capacidad_toneladas','certificacion_ambiental','area_impacto','tecnologia_clasificacion'],
        ...
        try {
            const res = await fetch('/api/auth/register', {
                ...
                body: JSON.stringify({
                    ...
                    repNombre: data.repNombre, repApellido: data.repApellido,
                    repCedula: data.repCedula, rep_cargo: data.rep_cargo,
                    rep_telefono: data.rep_telefono, repEmail: data.repEmail,
                    password: data.password, email_verificado: verifMethod === 'email',
                    telefono_verificado: verifMethod === 'sms', modules: MODULES_SOSTENIBILIDAD,
                }),
            });
            ...
            <Card className="w-full border-none shadow-2xl bg-card/60 backdrop-blur-xl rounded-[2rem] overflow-hidden">
                <CardHeader className="p-8 border-b border-border/50 bg-muted/5">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <CardTitle className="text-sm font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Registro · Sostenibilidad</CardTitle>
                            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-0.5">Ameru IA · Eco-Créditos · Gestión Ambiental</CardDescription>
                        </div>
                    </div>
                    ...
                    <CardContent className="p-8 space-y-5">
                        ...
                        {step === 1 && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="sm:col-span-2 space-y-2">
                                    <Label className="text-[10px] font-black uppercase tracking-widest">Razón Social / Nombre Organización *</Label>
                                    <Input placeholder="Empresa o Asociación..." {...register('razonSocial')} className={cn(errors.razonSocial && 'border-destructive')} />
                                ...
                                <div className="sm:col-span-2 space-y-2">
                                    <Label className="text-[10px] font-black uppercase tracking-widest">Dirección *</Label>
                            ...
                            <div className="space-y-4">
                                <div className="p-4 bg-emerald-500/5 rounded-xl border border-emerald-500/10 mb-2">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-1 flex items-center gap-2">
                                        <Leaf className="h-3.5 w-3.5" /> Perfil Ambiental
                                    </p>
                                    <p className="text-[9px] text-muted-foreground">Esta información configura los módulos de gestión de residuos, Eco-Créditos y la IA de clasificación Ameru.</p>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase tracking-widest">Tipo de Residuos Principales *</Label>
                                        <Controller name="tipo_residuos" control={control} render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                <SelectTrigger className={cn(errors.tipo_residuos && 'border-destructive')}><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                                                <SelectContent>{TIPOS_RESIDUOS.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                                            </Select>
                                        )} />
                                        {errors.tipo_residuos && <p className="text-[10px] text-destructive">{errors.tipo_residuos.message}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase tracking-widest">Capacidad Mensual (Toneladas) *</Label>
                                        <Input type="number" min="0" step="0.1" placeholder="Ej: 150" {...register('capacidad_toneladas')} className={cn(errors.capacidad_toneladas && 'border-destructive')} />
                                        {errors.capacidad_toneladas && <p className="text-[10px] text-destructive">{errors.capacidad_toneladas.message}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase tracking-widest">Certificación Ambiental *</Label>
                                        <Controller name="certificacion_ambiental" control={control} render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                <SelectTrigger className={cn(errors.certificacion_ambiental && 'border-destructive')}><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                                                <SelectContent>{CERTIFICACIONES.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                                            </Select>
                                        )} />
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase tracking-widest">Área de Impacto *</Label>
                                        <Controller name="area_impacto" control={control} render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                <SelectTrigger className={cn(errors.area_impacto && 'border-destructive')}><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                                                <SelectContent>{AREAS_IMPACTO.map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}</SelectContent>
                                            </Select>
                                        )} />
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase tracking-widest">Tecnología de Clasificación *</Label>
                                        <Controller name="tecnologia_clasificacion" control={control} render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                <SelectTrigger className={cn(errors.tecnologia_clasificacion && 'border-destructive')}><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                                                <SelectContent>{TECNOLOGIAS_CLASIFICACION.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                                            </Select>
                                        )} />
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase tracking-widest">Objetivo Eco-Créditos/Mes</Label>
                                        <Input type="number" min="0" placeholder="Ej: 500 créditos" {...register('eco_creditos_objetivo')} />
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {step === 3 && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[
                                    ...
                                    { id: 'rep_cargo', label: 'Cargo *', placeholder: 'Ej: Director Ambiental, Gerente...' },
                                    { id: 'rep_telefono', label: 'Teléfono *' },
                                    { id: 'repEmail', label: 'Correo *', type: 'email' },
                                ].map(f => (
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            {(['email','sms'] as const).map(m => (
                                                <button key={m} type="button" onClick={() => setVerifMethod(m)}
                                                    className={cn("p-4 rounded-xl border text-xs font-black uppercase tracking-widest transition-all", verifMethod === m ? "border-emerald-500 bg-emerald-500/5 text-emerald-600" : "border-border text-muted-foreground")}>
                                                    {m === 'email' ? '📧 Correo' : '📱 SMS'}
                                                </button>
                                            ))}
                                        </div>
                        ...
                        {step === TOTAL_STEPS && (
                            <div className="text-center py-8 space-y-4">
                                ...
                                <h2 className="text-2xl font-black uppercase italic tracking-tight">¡Perfil Ambiental Creado!</h2>
                                <p className="text-muted-foreground text-sm">Tu organización fue registrada en el módulo de <strong className="text-emerald-600">Sostenibilidad Ameru IA</strong>.</p>
                                <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl text-left text-xs space-y-2">
                                    <p className="font-black text-emerald-600 uppercase tracking-widest">Módulos habilitados:</p>
                                    {MODULES_SOSTENIBILIDAD.map(m => <p key={m.id} className="text-muted-foreground">✓ {m.label}</p>)}
                                </div>
                                <Button className="w-full mt-4 bg-emerald-600 hover:bg-emerald-700" onClick={() => { localStorage.setItem('kyron-just-registered','true'); router.push('/'); }}>
                                    Ir al Portal Ambiental<ArrowRight className="ml-2 h-4 w-4"/>
                                </Button>
                            </div>
                        )}
                    </CardContent>
                        ...
                        <CardFooter className="flex justify-between p-8 pt-2">
                            ...
                            {step === 4 && (
                                <Button type="submit" disabled={isLoading || !verifVerified}>
                                    {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Registrando...</> : <>Crear Perfil Ambiental<ArrowRight className="ml-2 h-4 w-4"/></>}
                                </Button>
                            )}
                        </CardFooter>
                    )}
