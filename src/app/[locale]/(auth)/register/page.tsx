...
import { User, Building2, ArrowRight, ChevronLeft, ShieldCheck } from "lucide-react";
...
const registerOptions = [
    ...
    },
    {
        ...
        description: 'Registro empresarial para organizaciones, empresas y entidades. Gestiona módulos y accesos corporativos.',
        badge: 'Empresa',
    },
];
...
export default function RegisterSelectionPage() {
    return (
        <div className="container mx-auto px-4 py-8 md:py-16 min-h-screen flex flex-col items-center max-w-4xl">
            <motion.div
                className="w-full"
                initial="hidden"
                animate="visible"
            ...
            >
                <header className="flex flex-col items-center text-center mb-12 md:mb-16">
                    ...
                    <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/5 border border-primary/10 text-primary text-[9px] font-black uppercase tracking-[0.2em] mb-6 shadow-glow-sm">
                        <ShieldCheck className="h-3.5 w-3.5" /> Registro Seguro
                    </motion.div>
                    ...
                    <motion.p variants={itemVariants} className="text-base md:text-lg text-muted-foreground max-w-xl text-balance leading-snug font-medium uppercase tracking-tight opacity-60">
                        Selecciona el tipo de cuenta que deseas registrar en el ecosistema.
                    </motion.p>
                </header>
                            ...
                            <Card className="h-full group hover:border-primary/40 transition-all duration-300 bg-card/40 backdrop-blur-xl flex flex-col border-border/50 shadow-lg hover:shadow-primary/5 rounded-[1.5rem] overflow-hidden">
                                <Link href={option.href as any} className="flex flex-col h-full">
                                    <CardHeader className="flex-row items-center gap-4 p-6 pb-4">
                                        ...
                                        <div>
                                            <CardTitle className="text-lg font-black group-hover:text-primary transition-colors tracking-tight leading-tight uppercase italic">
                                                {option.label}
                                            </CardTitle>
                                    ...
                                    <CardContent className="flex-grow px-6 pb-6">
                                        <CardDescription className="text-xs md:text-sm leading-snug text-muted-foreground font-medium uppercase tracking-tight">
                                            {option.description}
                                        </CardDescription>
                                    </CardContent>
                                    <div className="p-4 pt-0 mt-auto border-t border-border/10">
                                        <span className="text-[8px] font-black uppercase tracking-[0.2em] text-primary/40 group-hover:text-primary inline-flex items-center transition-all duration-300">
                                            Registrarse <ArrowRight className="h-3.5 w-3.5 ml-1.5" />
                                        </span>
                                    </div>
                                </Link>
                            </Card>
                ...
                <motion.p variants={itemVariants} className="text-center text-xs text-muted-foreground mt-10">
