    ...
    while (remaining.length > 0) {
      ...
      if (codeMatch && typeof codeMatch.index === 'number') {
        if (!firstMatch || codeMatch.index < firstMatch.idx) {
          firstMatch = { idx: codeMatch.index, len: codeMatch[0].length, node: <code key={key++} className="px-1.5 py-0.5 rounded-md bg-muted text-xs font-mono text-primary">{codeMatch[1]}</code> };
        }
      }
