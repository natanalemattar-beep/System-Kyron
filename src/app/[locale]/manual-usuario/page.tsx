...
const introSection = {
    title: "1.0 Bienvenido a System Kyron v2.7.0",
    mission: "Garantizar la soberanía tecnológica, fiscal y sustentable mediante arquitecturas inmutables de grado corporativo.",
    target: "Empresas de alto rendimiento, instituciones de misión crítica y ciudadanos digitales.",
    text: "System Kyron v2.7.0 representa la cumbre de la integración operativa. No es solo un software de gestión; es un nodo de inteligencia convergente que fusiona telecomunicaciones 5G, ciberseguridad cuántica, fiscalidad predictiva y una economía circular basada en activos verdes tokenizados."
};
    ...
    const handleDownload = () => {
        const content = `
            <div style="text-align: center; margin-bottom: 60pt; border: 4pt double #0ea5e9; padding: 40pt; border-radius: 20pt; background-color: #f0f9ff;">
                ...
                <p style="color: #1e293b; font-size: 18pt; font-weight: bold; text-transform: uppercase; letter-spacing: 5pt; margin-bottom: 30pt;">Expediente Técnico Maestro v2.7.0</p>
            ...
            <h2 style="color: #0ea5e9; margin-top: 50pt; border-left: 15pt solid #0ea5e9; padding-left: 15pt; font-family: 'Arial Black';">2.0 INNOVACIONES DE GRADO SUPERIOR (TIER 2)</h2>
            ...
            <h2 style="color: #0ea5e9; margin-top: 50pt; border-left: 15pt solid #0ea5e9; padding-left: 15pt; font-family: 'Arial Black';">3.0 PROTOCOLOS POR MÓDULO</h2>
            ${manualModules.map(mod => `
                <div style="margin-top: 20pt; padding: 20pt; border: 1pt solid #e2e8f0; border-radius: 10pt;">
                    ...
                    <p>${mod.description}</p>
                    <p><strong>PROCEDIMIENTO:</strong> ${mod.procedure}</p>
                </div>
            `).join('')}
        `;

        const headerHtml = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Manual Maestro Kyron</title></head><body style='padding: 50pt; font-family: Arial, sans-serif;'>";
        const footerHtml = "</body></html>";
        const blob = new Blob([headerHtml + content + footerHtml], { type: 'application/msword' });
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.download = "MANUAL_MAESTRO_KYRON_V2.7.0.doc";
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);

        toast({ title: "EXPEDIENTE EXPORTADO", description: "Manual v2.7.0 generado con éxito." });
    };
            ...
            <header className="fixed top-0 left-0 right-0 z-[150] h-16 bg-black/90 backdrop-blur-3xl border-b border-white/5 flex items-center px-6 md:px-12 justify-between shadow-glow">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col border-l border-white/10 pl-4">
                        ...
                        <span className="text-[8px] font-bold text-primary uppercase tracking-[0.3em] opacity-60">Manual Maestro v2.7.0</span>
                    </div>
                </div>
                    ...
                    <aside className="lg:col-span-4 no-print">
                        <Card className="glass-card p-6 rounded-[2.5rem] sticky top-24 border-white/5 bg-black/60 shadow-2xl">
                            ...
                            <CardContent className="p-0 space-y-1">
                                ...
                                <div className="py-4 px-3 text-[7px] font-black uppercase tracking-[0.5em] text-white/20 italic">Innovaciones v2.7</div>
                                {innovationModules.map(mod => (
                    ...
                    <div className="lg:col-span-8 space-y-32">
                        <section id="intro" className="space-y-8 scroll-mt-24">
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-[8px] font-black uppercase tracking-[0.5em] text-primary shadow-glow mb-6">
                                <Sparkles className="h-3 w-3" /> NODO CENTRAL v2.7
                            </div>
            ...
            <Card className="glass-card rounded-[3.5rem] border-white/5 overflow-hidden bg-black/60 shadow-2xl transition-all hover:border-primary/20">
                ...
                <CardFooter className="p-12 border-t border-white/5 flex justify-between items-center bg-white/[0.01]">
                    <div className="flex items-center gap-3 text-[8px] font-black uppercase tracking-widest text-white/20">
                        <ShieldCheck className="h-3.5 w-3.5" /> SECURE NODE v2.7
                    </div>
                    <Badge variant="outline" className="border-white/10 text-white/40 text-[8px] font-black px-4 py-1.5 rounded-lg">VERIFIED BY KYRON CORP</Badge>
                </CardFooter>
            </Card>
        </section>
    );
