...
async function getOrCreateConfig(userId: number) {
  ...
  let config = await queryOne(
    `SELECT id, idioma, moneda_preferida, zona_horaria,
            ...
            notif_sms, telefono_sms, notif_push,
  ...
  const [config] = await query(
    `UPDATE configuracion_usuario
     SET idioma             = COALESCE($1,  idioma),
         ...
         notif_push         = COALESCE($9,  notif_push),
         notif_vencimientos = COALESCE($10, notif_vencimientos),
         notif_pagos        = COALESCE($11, notif_pagos),
         iva_pct            = COALESCE($12, iva_pct),
         igtf_pct           = COALESCE($13, igtf_pct),
         islr_pct           = COALESCE($14, islr_pct),
         rif_empresa        = COALESCE($15, rif_empresa),
         nombre_comercial   = COALESCE($16, nombre_comercial),
         logo_url           = COALESCE($17, logo_url),
         pie_factura        = COALESCE($18, pie_factura),
         email_verificacion = COALESCE($19, email_verificacion),
         email_alertas      = COALESCE($20, email_alertas),
         reducir_animaciones = COALESCE($21, reducir_animaciones),
         nav_lateral        = COALESCE($22, nav_lateral),
         updated_at         = NOW()
     WHERE user_id = $23
     RETURNING *`,
    [
      ...
      telefono_sms ?? null,
      notif_push !== undefined ? notif_push : null,
