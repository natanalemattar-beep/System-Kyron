import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM cumplimiento_normativo WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) FILTER (WHERE estado='cumplido') as cumplidos, COUNT(*) FILTER (WHERE estado='pendiente') as pendientes, COUNT(*) FILTER (WHERE nivel_riesgo='alto' OR nivel_riesgo='critico') as alto_riesgo, COUNT(*) as total FROM cumplimiento_normativo WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.norma, body.organismo, body.categoria, body.descripcion, body.nivel_riesgo, body.fecha_limite, body.responsable, body.multa_estimada, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM cumplimiento_normativo WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
