import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM litigios WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) FILTER (WHERE estado='en_curso') as en_curso, COUNT(*) FILTER (WHERE estado='ganado') as ganados, COUNT(*) FILTER (WHERE estado='perdido') as perdidos, COUNT(*) as total FROM litigios WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.numero_expediente, body.tribunal, body.tipo, body.demandante, body.demandado, body.materia, body.abogado_responsable, body.monto_demanda, body.moneda, body.fecha_inicio, body.proxima_audiencia, body.prioridad, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM litigios WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
