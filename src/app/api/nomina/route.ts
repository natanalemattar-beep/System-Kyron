...
export async function POST(req: NextRequest) {
  ...
  const [nomina] = await query<{ id: number; periodo: string }>(
    ...
    [
      ...
      fecha_pago ?? null,
      tipo ?? 'quincenal',
      parseFloat(total_asignaciones ?? '0'),
      parseFloat(total_deducciones ?? '0'),
      parseFloat(total_neto ?? '0'),
      estado ?? 'pendiente',
      notas ?? null,
    ]
  );
    ...
    for (const item of items as Record<string, unknown>[]) {
      await query(
        ...
        [
          nomina.id,
          item.empleado_id,
          parseFloat((item.salario_base as string) ?? '0'),
          parseFloat((item.dias_trabajados as string) ?? '30'),
          parseFloat((item.horas_extras as string) ?? '0'),
          parseFloat((item.bono_productividad as string) ?? '0'),
          parseFloat((item.cestaticket as string) ?? '0'),
          parseFloat((item.utilidades as string) ?? '0'),
          parseFloat((item.vacaciones as string) ?? '0'),
          parseFloat((item.total_asignaciones as string) ?? '0'),
          parseFloat((item.sso as string) ?? '0'),
          parseFloat((item.lph as string) ?? '0'),
          parseFloat((item.faov as string) ?? '0'),
          parseFloat((item.rpe as string) ?? '0'),
          parseFloat((item.islr_retenido as string) ?? '0'),
          parseFloat((item.otros_descuentos as string) ?? '0'),
          parseFloat((item.total_deducciones as string) ?? '0'),
          parseFloat((item.neto as string) ?? '0'),
        ]
      );
    }
  }
