...
export async function POST(req: NextRequest) {
  ...
  const result = await query(`
    ...
    RETURNING *
  `, [session.user.id, proveedor, pais_origen, descripcion, referencia_orden, monto || 0, moneda || 'USD', tasa_cambio, monto_bs, flete || 0, seguro || 0, aranceles || 0, iva_aduanero || 0, costoTotal, fecha_orden, fecha_embarque, fecha_llegada, incoterm || 'FOB', estado || 'pendiente', agente_aduanal, numero_bl, numero_dua, notas]);

  return NextResponse.json({ compra: result[0] }, { status: 201 });
}
...
export async function PUT(req: NextRequest) {
  ...
  return NextResponse.json({ compra: result[0] });
}
...
export async function DELETE(req: NextRequest) {
  ...
  await query('DELETE FROM compras_internacionales WHERE id = $1 AND user_id = $2', [id, session.user.id]);
