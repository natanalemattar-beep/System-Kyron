...
export async function GET() {
  ...
  try {
    const proyectos = await query(`SELECT * FROM proyectos_personal WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rows = await query(
      ...
      [session.user.id, titulo, departamento, tipo || 'estrategia', descripcion || '', objetivo || '', responsable || '', fecha_inicio, fecha_fin_est || null, prioridad || 'media', presupuesto || 0, estado || 'planificado', progreso || 0]
    );
