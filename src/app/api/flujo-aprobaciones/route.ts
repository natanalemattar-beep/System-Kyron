import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM flujo_aprobaciones WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE estado='pendiente') as pendientes, COUNT(*) FILTER (WHERE estado='aprobado') as aprobados, COUNT(*) FILTER (WHERE estado='rechazado') as rechazados FROM flujo_aprobaciones WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.tipo, body.titulo, body.descripcion, body.solicitante, body.departamento, body.monto, body.moneda, body.prioridad]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM flujo_aprobaciones WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
