import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM cobros WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE estado='pendiente') as pendientes, COUNT(*) FILTER (WHERE estado='cobrado') as cobrados, COALESCE(SUM(saldo_pendiente),0) as saldo_total FROM cobros WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.factura_ref, body.cliente_nombre, body.cliente_rif, body.monto_original, body.moneda, body.fecha_vencimiento, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM cobros WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
