import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM vehiculos WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE estado='activo') as activos, COUNT(*) FILTER (WHERE seguro_vencimiento < CURRENT_DATE) as seguro_vencido, COUNT(*) FILTER (WHERE revision_tecnica < CURRENT_DATE) as revision_vencida FROM vehiculos WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.marca, body.modelo, body.anio, body.color, body.placa, body.serial_carroceria, body.serial_motor, body.tipo, body.fecha_registro, body.seguro_poliza, body.seguro_vencimiento, body.revision_tecnica, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM vehiculos WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
