import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM ordenes_compra WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE estado='pendiente') as pendientes, COUNT(*) FILTER (WHERE estado='recibida') as recibidas, COALESCE(SUM(total),0) as monto_total FROM ordenes_compra WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.numero_orden, body.proveedor_nombre, body.proveedor_rif, body.fecha_emision, body.fecha_entrega_esperada, body.moneda, body.departamento, body.condiciones, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM ordenes_compra WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
