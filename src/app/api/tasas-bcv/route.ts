...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[tasas-bcv] GET error:', err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const usdVal = parseFloat(tasa_usd_ves);
    if (isNaN(usdVal) || usdVal <= 0) {
      return NextResponse.json({ error: 'Tasa USD/VES inválida' }, { status: 400 });
    }
    ...
    console.error('[tasas-bcv] POST error:', err);
