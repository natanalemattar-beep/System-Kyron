...
export async function POST(req: NextRequest) {
  ...
  if (impuesto === 'islr') {
    ...
    const [decl] = await query(
      ...
      [
        ...
        parseFloat(enriquecimiento_neto ?? '0'),
        parseFloat(impuesto_causado ?? '0'),
        parseFloat(anticipo_pagado ?? '0'),
        parseFloat(impuesto_a_pagar ?? '0'),
        estado ?? 'pendiente',
  ...
  const debitoNum  = parseFloat(iva_debito  ?? '0');
  const creditoNum = parseFloat(iva_credito ?? '0');
