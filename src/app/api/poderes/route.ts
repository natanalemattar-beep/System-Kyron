import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM poderes_notariados WHERE user_id = $1 ORDER BY created_at DESC`, [session.userId]);
      const statsResult = await query(`SELECT COUNT(*) FILTER (WHERE estado='vigente') as vigentes, COUNT(*) FILTER (WHERE estado='vencido') as vencidos, COUNT(*) FILTER (WHERE estado='revocado') as revocados, COUNT(*) as total FROM poderes_notariados WHERE user_id = $1`, [session.userId]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.userId, body.tipo, body.titulo, body.otorgante, body.apoderado, body.cedula_otorgante, body.cedula_apoderado, body.notaria, body.numero_documento, body.tomo, body.folio, body.facultades, body.fecha_otorgamiento, body.fecha_vencimiento, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM poderes_notariados WHERE id = $1 AND user_id = $2`, [id, session.userId]);
