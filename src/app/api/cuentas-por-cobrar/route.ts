...
export async function POST(req: NextRequest) {
  ...
  const [cxc] = await query<{ id: number }>(
    `INSERT INTO cuentas_por_cobrar (user_id, cliente_id, factura_id, concepto, monto_original, monto_pendiente, moneda, fecha_emision, fecha_vencimiento, notas)
     ...
     RETURNING id`,
    [session.user.id, cliente_id ?? null, factura_id ?? null, concepto, monto, moneda ?? 'VES', fecha_emision, fecha_vencimiento ?? null, notas ?? null]
  );
  ...
  await logActivity({
    ...
    descripcion: `CÃ—C registrada: ${concepto} â€” Bs. ${monto.toFixed(2)}`,
    entidadTipo: 'cxc',
    entidadId: cxc.id,
  });
