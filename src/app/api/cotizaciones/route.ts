import { NextRequest, NextResponse } from "next/server";
  ...
  export async function GET() {
    try {
      ...
      const rows = await query(`SELECT * FROM cotizaciones WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
      const statsResult = await query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE estado='enviada') as enviadas, COUNT(*) FILTER (WHERE estado='aceptada') as aceptadas, COALESCE(SUM(total),0) as monto_total FROM cotizaciones WHERE user_id = $1`, [session.user.id]);
  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const result = await query(
        ...
        [session.user.id, body.numero_cotizacion, body.cliente_nombre, body.cliente_rif, body.cliente_email, body.fecha_emision, body.fecha_validez, body.moneda, body.condiciones, body.notas]
      );
  ...
  export async function DELETE(req: NextRequest) {
    try {
      ...
      await query(`DELETE FROM cotizaciones WHERE id = $1 AND user_id = $2`, [id, session.user.id]);
