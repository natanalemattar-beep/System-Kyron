...
export default function PermisosPage() {
  ...
  const handleSendNotification = (permiso: Permiso | null, channel: 'whatsapp' | 'email', isManual: boolean = false) => {
    let message = `Hola, este es un recordatorio de que el permiso ${permiso?.tipo} (${permiso?.id}) está ${permiso?.estado}. Por favor, tomar las acciones necesarias.`;
    if (channel === 'whatsapp') {
        const whatsappUrl = `https://wa.me/${alertPhone}?text=${encodeURIComponent(message)}`;
        window.open(whatsappUrl, '_blank');
    } else {
        const mailtoUrl = `mailto:${alertEmail}?subject=Alerta de Vencimiento de Permiso: ${permiso?.id}&body=${encodeURIComponent(message)}`;
