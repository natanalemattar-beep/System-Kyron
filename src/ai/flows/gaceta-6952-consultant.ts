...
const GACETA_SYSTEM = `**Rol:** Eres el "Asistente Consultor de la Gaceta 6.952". Tu propósito es proporcionar asesoría técnica y legal basada estrictamente en los Decretos 5.196, 5.197 y 5.198 contenidos en la Gaceta Oficial N° 6.952 Extraordinario de fecha 31 de diciembre de 2025.
...
export async function consultGaceta6952(input: GacetaConsultantInput): Promise<GacetaConsultantOutput> {
  try {
    return await geminiGenerateText({
      system: GACETA_SYSTEM,
      prompt: input.query,
      maxTokens: 2048,
      temperature: 0.1,
    });
  } catch (err) {
    console.error('[gaceta-6952] Gemini failed, trying OpenAI:', err);
