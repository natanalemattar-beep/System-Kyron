  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
      {stats.map((stat, index) => (
        <Card key={stat.title} className="glass-card border-none group relative overflow-hidden rounded-xl">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-[8px] font-black uppercase tracking-[0.3em] text-white/30">{stat.title}</CardTitle>
            <div className={cn("p-1.5 rounded-lg border border-white/5", stat.bg, stat.color)}>
                <stat.icon className="h-3.5 w-3.5" />
            </div>
          </CardHeader>
          <CardContent className="pb-4">
            <div className="text-xl font-black tracking-tighter italic text-white mb-0.5">
