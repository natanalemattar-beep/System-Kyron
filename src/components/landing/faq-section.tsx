...
export function FaqSection() {
    ...
    return (
        <section className="py-24 relative overflow-hidden bg-gray-50 dark:bg-[#03050a]">
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="max-w-3xl mx-auto text-center mb-16">
                    ...
                    <p className="text-gray-600 dark:text-slate-400 text-lg font-medium max-w-2xl mx-auto">
                        {t('subtitle')}
                    </p>
                </div>
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="relative mb-12 group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-500 dark:text-slate-500 group-focus-within:text-cyan-400 transition-colors" />
                        <Input 
                            type="text"
                            placeholder={t('search_placeholder')}
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            className="h-14 pl-12 bg-gray-100 dark:bg-white/5 border-gray-200 dark:border-white/10 rounded-2xl focus:border-cyan-500/50 focus:ring-cyan-500/20 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-600 transition-all"
                        />
                    </div>
                    ...
                    <Accordion type="single" collapsible className="space-y-4">
                        {filteredItems.map((item, index) => (
                            <AccordionItem 
                                key={index} 
                                value={`item-${index}`}
                                className="border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-white/[0.02] rounded-2xl px-6 overflow-hidden hover:bg-gray-100 dark:hover:bg-white/[0.04] transition-all"
                            >
                                ...
                                <AccordionContent className="pb-6 pt-2 text-gray-600 dark:text-slate-400 text-base leading-relaxed border-t border-gray-200 dark:border-white/5">
                                    <div className="pl-14">
                                        {item.answer}
                    ...
                    {filteredItems.length === 0 && (
                        <div className="text-center py-20 bg-gray-50 dark:bg-white/[0.02] rounded-3xl border border-dashed border-gray-200 dark:border-white/10">
                            <HelpCircle className="h-12 w-12 text-gray-400 dark:text-slate-700 mx-auto mb-4" />
                            <p className="text-gray-500 dark:text-slate-500 font-bold uppercase tracking-widest text-sm">
                                No se encontraron resultados para "{search}"
                            </p>
                            <p className="text-gray-500 dark:text-slate-600 text-sm mt-2">Prueba preguntándole directamente a nuestro asistente AI abajo.</p>
                        </div>
                    )}
                    ...
                    <div className="mt-16 p-8 rounded-3xl border border-cyan-500/10 bg-gradient-to-br from-cyan-500/5 to-blue-500/5">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          <h3 className="text-gray-900 dark:text-white font-bold text-lg">¿No encuentras lo que buscas?</h3>
                          <p className="text-gray-600 dark:text-slate-400 text-sm">Pregúntale directamente a nuestro asistente AI</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 h-12 px-4 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm placeholder:text-gray-400 dark:placeholder:text-slate-600 focus:outline-none focus:border-cyan-500/50"
                        />
                        <Button
                          onClick={askAI}
                      ...
                      {aiReply && (
                        <div className="mt-4 p-4 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5">
