...
export function AIBudgetPlanner() {
    ...
    return (
        <section className="py-24 md:py-32 relative overflow-hidden bg-[#050816]">
            {/* Background Effects */}
            ...
            <div className="container mx-auto px-6 max-w-7xl relative z-10">
                <ScrollReveal className="text-center mb-16 space-y-6">
                    <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 backdrop-blur-md mx-auto">
                ...
                <div className="grid lg:grid-cols-12 gap-12 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-10">
                        <div className="glass-system-kyron-interactive p-8 md:p-10 rounded-[2.5rem] border-white/5 space-y-8">
                            <div className="space-y-6">
                                <label className="text-[10px] font-black uppercase tracking-[0.25em] text-white/30 flex items-center gap-2">
                                    <DollarSign className="h-3 w-3" /> {t('input_label')}
                                </label>
                                <div className="space-y-6">
                                    <Slider 
                                        value={[budget]} 
                                    ...
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-black text-white/20">$5</span>
                                        <span className="text-5xl font-black text-white tracking-tighter">${budget}</span>
                                        <span className="text-sm font-black text-white/20">$300</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <label className="text-[10px] font-black uppercase tracking-[0.25em] text-white/30 flex items-center gap-2">
                                    <Zap className="h-3 w-3" /> {t('needs_label')}
                                </label>
                                    ...
                                    {Object.entries(t.raw('needs')).map(([key, label]: [string, any]) => (
                                        ...
                                        >
                                            <div className={cn(
                                                ...
                                                needs.includes(key) ? "bg-cyan-500/20 text-cyan-400" : "bg-white/5 text-white/20"
                                            )}>
                                                {(() => {
                                            ...
                                            {needs.includes(key) && <Check className="h-4 w-4 ml-auto text-cyan-400" />}
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <Button 
                                ...
                                className="w-full h-16 rounded-2xl bg-white text-black hover:bg-zinc-200 font-black text-[11px] uppercase tracking-[0.3em] shadow-[0_20px_40px_rgba(255,255,255,0.1)] active:scale-95 transition-all group"
                            >
                                {isGenerating ? (
                                    <Loader2 className="h-5 w-5 animate-spin mr-3" />
                                ) : (
                                    <Sparkles className="h-5 w-5 mr-3 text-cyan-600" />
                                )}
                                {t('btn_generate')}
                            </Button>
                        </div>
                        ...
                        <AnimatePresence mode="wait">
                            {isGenerating ? (
                                <motion.div 
                                    ...
                                    className="h-full flex flex-col items-center justify-center space-y-8 glass-system-kyron-interactive rounded-[2.5rem] border-white/5 p-12 text-center"
                                >
                                    ...
                                    <div className="space-y-4 max-w-md">
                                        <p className="text-sm font-black text-white uppercase tracking-widest leading-relaxed">
                                            {t('ai_thinking')}
                                        </p>
                                ...
                                >
                                    <div className="glass-system-kyron-interactive rounded-[2.5rem] border-cyan-500/20 bg-cyan-500/5 p-10 md:p-12 relative overflow-hidden h-full">
                                        <div className="absolute top-0 right-0 p-8">
                                            <ShieldCheck className="h-16 w-16 text-cyan-500/10" />
                                        </div>
                                        ...
                                        <div className="space-y-2 mb-10">
                                            ...
                                            <p className="text-sm text-white/40 font-medium">
                                                {t('result_subtitle')} <span className="text-white font-black">${budget}/mes</span>
                                            </p>
                                        </div>
                                        ...
                                        <div className="grid gap-4 mb-12">
                                            {recommendation.modules.map((modId) => (
                                                <motion.div 
                                                    ...
                                                    className="flex items-center gap-5 p-5 rounded-3xl bg-white/[0.03] border border-white/5 hover:border-white/10 transition-colors"
                                                >
                                                    ...
                                                    <div className="flex-1">
                                                        <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-0.5">Módulo Activo</p>
                                        ...
                                        <div className="grid md:grid-cols-2 gap-6 pt-10 border-t border-white/5">
                                            <div className="space-y-1">
                                                <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em]">{t('total_value')}</p>
                                                <p className="text-3xl font-black text-white tracking-tighter">${recommendation.totalValue}</p>
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-[9px] font-black text-emerald-500/40 uppercase tracking-[0.3em]">{t('savings')}</p>
                                                <p className="text-3xl font-black text-emerald-400 tracking-tighter">${recommendation.savings}</p>
                                            </div>
                                        </div>
                                ...
                                <motion.div 
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    className="h-full flex flex-col items-center justify-center space-y-6 glass-system-kyron-interactive rounded-[2.5rem] border-white/5 border-dashed border-2 opacity-30"
                                >
                                    <div className="h-20 w-20 rounded-full bg-white/5 flex items-center justify-center">
                                        <Cpu className="h-10 w-10 text-white/20" />
                                    </div>
                                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40">Esperando parámetros...</p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
