...
type CertType = 'dependiente' | 'independiente' | 'jubilado' | null;
        ...
        <div className="space-y-10">
            {step === 'selector' && (
                <div className="grid md:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <Card onClick={() => handleSelectType('dependiente')} className="glass-card border-none p-10 cursor-pointer group hover:bg-primary/5 transition-all text-center">
                        <div className="p-6 bg-primary/10 rounded-[2rem] w-fit mx-auto mb-8 border border-primary/20 group-hover:scale-110 transition-transform">
                            <User className="h-10 w-10 text-primary" />
                        </div>
                        <h3 className="text-xl font-black uppercase italic tracking-tighter mb-4">Trabajador Dependiente</h3>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase leading-relaxed mb-8">Generar constancia de trabajo oficial con datos corporativos.</p>
                    ...
                    <Card onClick={() => handleSelectType('independiente')} className="glass-card border-none p-10 cursor-pointer group hover:bg-secondary/5 transition-all text-center">
                        <div className="p-6 bg-secondary/10 rounded-[2rem] w-fit mx-auto mb-8 border border-secondary/20 group-hover:scale-110 transition-transform">
                        ...
                        <p className="text-[10px] font-bold text-muted-foreground uppercase leading-relaxed mb-8">Informe de atestiguamiento para el libre ejercicio profesional.</p>
                    ...
                    <Card onClick={() => handleSelectType('jubilado')} className="glass-card border-none p-10 cursor-pointer group hover:bg-rose-500/5 transition-all text-center">
                        <div className="p-6 bg-rose-500/10 rounded-[2rem] w-fit mx-auto mb-8 border border-rose-500/20 group-hover:scale-110 transition-transform">
                            <Heart className="h-10 w-10 text-rose-500" />
                        </div>
                        <h3 className="text-xl font-black uppercase italic tracking-tighter mb-4">Jubilado / Pensionado</h3>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase leading-relaxed mb-8">Constancia de jubilación o pensión ante el IVSS o privado.</p>
                ...
                <Card className="glass-card border-none rounded-[3rem] p-1 shadow-2xl animate-in fade-in zoom-in-95 duration-500 max-w-4xl mx-auto">
                    <CardHeader className="p-10 border-b border-border/50">
                        <div className="flex items-center gap-6">
                            <Button variant="ghost" onClick={() => setStep('selector')} className="h-10 w-10 rounded-full bg-muted/50"><ArrowLeft className="h-4 w-4" /></Button>
                            <div>
                    ...
                    <CardContent className="p-10 space-y-8">
                        {mode === 'hr' && (
                            <div className="space-y-3">
                                ...
                                <Select onValueChange={handleEmployeeSelect}>
                                    <SelectTrigger className="h-12 rounded-xl bg-white/5 border-border font-bold"><SelectValue placeholder="Seleccione un trabajador..." /></SelectTrigger>
                                    <SelectContent className="rounded-xl">
                                        {mockEmployees.map(e => <SelectItem key={e.id} value={e.id}>{e.name} ({e.ci})</SelectItem>)}
                                    </SelectContent>
                                </Select>
                            </div>
                        )}
                        ...
                        <div className="grid md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                ...
                                <Input value={formData.nombre || ""} readOnly={mode === 'personal'} onChange={e => setFormData({...formData, nombre: e.target.value})} className="h-12 rounded-xl bg-white/5 border-border font-bold" />
                            </div>
                            <div className="space-y-2">
                                <Label className="text-[9px] font-black uppercase text-muted-foreground/40 ml-1">Cédula de Identidad</Label>
                                <Input value={formData.cedula || ""} readOnly={mode === 'personal'} onChange={e => setFormData({...formData, cedula: e.target.value})} className="h-12 rounded-xl bg-white/5 border-border font-bold" />
                            </div>
                            <div className="space-y-2">
                                <Label className="text-[9px] font-black uppercase text-muted-foreground/40 ml-1">Empresa / Ente</Label>
                            ...
                            <div className="space-y-2">
                                <Label className="text-[9px] font-black uppercase text-muted-foreground/40 ml-1">Cargo / Actividad</Label>
                            ...
                            <div className="space-y-2">
                                <Label className="text-[9px] font-black uppercase text-muted-foreground/40 ml-1">Sueldo / Ingreso Mensual</Label>
                            ...
                            <div className="space-y-2">
                                <Label className="text-[9px] font-black uppercase text-muted-foreground/40 ml-1">Fecha de Ingreso</Label>
                    ...
                    <CardFooter className="p-10 bg-muted/10 border-t border-border flex justify-end">
            ...
            {step === 'preview' && (
                <div className="space-y-10 animate-in fade-in duration-700">
                    <Card className="max-w-4xl mx-auto bg-white p-12 md:p-20 shadow-[0_0_50px_rgba(0,0,0,0.1)] border border-slate-100 rounded-lg text-slate-950 font-serif relative overflow-hidden">
                        ...
                        <header className="flex justify-between items-start mb-20 border-b-2 border-slate-900 pb-10">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[8px] font-bold uppercase tracking-[0.4em] opacity-60">Control Maestro • RIF: J-12345678-9</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <h3 className="text-2xl font-black uppercase italic tracking-tighter text-slate-900">CERTIFICACIÓN</h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Expediente: {Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                            </div>
                        </header>

                        <div className="space-y-12 text-justify leading-loose text-lg">
                            <p className="indent-12">
                                Mediante la presente se hace constar que el(la) ciudadano(a) <strong className="uppercase">{formData.nombre}</strong>, titular de la Cédula de Identidad Nro. <strong>{formData.cedula}</strong>, presta sus servicios en la organización <strong>{formData.empresa}</strong> (RIF: {formData.rif || "N/A"}), desempeñando el cargo de <strong>{formData.cargo?.toUpperCase()}</strong>.
                            </p>
                            
                            <p className="indent-12">
                                Se certifica que el titular de la presente devenga un ingreso mensual de <strong>{formatCurrency(formData.salario || 0, 'Bs.')}</strong>, habiendo ingresado a la institución en fecha <strong>{formatDate(formData.ingreso)}</strong>.
                            </p>
                        ...
                        <footer className="mt-32 grid md:grid-cols-2 gap-20">
                            <div className="flex flex-col items-center">
                                ...
                                <p className="font-bold text-sm uppercase">Firma Autorizada</p>
                                <p className="text-[10px] uppercase opacity-40">System Kyron v2.6</p>
                            </div>
                            <div className="flex flex-col items-end">
                                <div className="p-4 border-2 border-slate-900 rounded-xl">
                                    <Image src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=VALID-KYRON-${formData.cedula}`} alt="QR Verification" width={80} height={80} className="grayscale" />
                                </div>
                                <p className="text-[8px] font-black uppercase tracking-widest mt-3 opacity-40">Sello de Integridad Inmutable</p>
                            </div>
                        </footer>

                        <div className="mt-20 pt-10 border-t border-slate-100 text-center">
                            <p className="text-[9px] font-black uppercase tracking-[0.5em] text-slate-300 italic">CERTIFICADO VÁLIDO POR 90 DÍAS • VALIDACIÓN BLOCKCHAIN ACTIVA</p>
                        </div>
                    </Card>

                    <div className="flex flex-wrap justify-center gap-4 no-print">
                        <Button variant="outline" className="rounded-xl h-12 px-8 font-black text-[10px] uppercase tracking-widest" onClick={() => setStep('form')}><ArrowLeft className="mr-2 h-4 w-4" /> Corregir Datos</Button>
                        <Button variant="outline" className="rounded-xl h-12 px-8 font-black text-[10px] uppercase tracking-widest" onClick={() => handleAction('Descarga PDF')}><Download className="mr-2 h-4 w-4" /> Bajar PDF</Button>
                        ...
                        <Button className="btn-3d-primary h-12 px-10 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-2xl" onClick={() => setStep('selector')}>FINALIZAR PROTOCOLO</Button>
                    </div>
                </div>
            )}
